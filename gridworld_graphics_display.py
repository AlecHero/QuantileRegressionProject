...
from graphics_util_pygame import GraphicsUtilGym, formatColor
...
class GraphicsGridworldDisplay:
    ...
    def annotate_text(self, state, symbol='o', color=(200,50, 50), dx=0, dy=0, action=None, fontsize=30, bold=False):
        ...
        if action is not None:
            dd = 0.2
            if action == NORTH:   y -= int( dd * self.GRID_SIZE)
            elif action == SOUTH: y += int( dd * self.GRID_SIZE)
            elif action == EAST: x += int(dd * self.GRID_SIZE)
            elif action == WEST: x -= int( dd * self.GRID_SIZE)
