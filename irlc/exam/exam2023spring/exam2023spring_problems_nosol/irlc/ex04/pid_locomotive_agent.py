...
class PIDLocomotiveAgent(Agent):
    def __init__(self, env: ControlEnvironment, dt, Kp=1.0, Ki=0.0, Kd=0.0, target=0):
        self.pid = PID(dt=dt, Kp=Kp, Ki=Ki, Kd=Kd, target=target) #!b # self.pid = PID(dt=...) #!b Make a pid instance here.
        super().__init__(env)

    def pi(self, x, k, info=None):
        u = self.pid.pi(x[0]) #!b #!b Get the correct action using self.pid.pi(...). Same as previous exercise
