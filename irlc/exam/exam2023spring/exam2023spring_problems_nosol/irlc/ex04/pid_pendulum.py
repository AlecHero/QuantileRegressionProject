...
if __name__ == "__main__":
    ...
    agent = PIDPendulumAgent(env, dt=env.dt, Kp=12, Ki=0, Kd=2, target_angle=0) #!b;halfquestionmarks #!b Define your agent here (including parameters)
    ...
    """
    env = get_offbalance_pendulum(30)
    agent = PIDPendulumAgent(env, dt=env.dt, Kp=12, Ki=2, Kd=2, target_angle=np.pi/6) #!b;halfquestionmarks #!b Define your agent here (include the target_angle parameter to the agent!)
    _, trajectories = train(env, agent, num_episodes=1, return_trajectory=True, reset=False)  # Note reset=False to maintain initial conditions.
    env.close()
    plot_trajectory(trajectories[0])
