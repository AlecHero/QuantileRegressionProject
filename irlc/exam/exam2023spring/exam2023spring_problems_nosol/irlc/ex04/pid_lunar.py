...
class ApolloLunarAgent(Agent):
    ...
    def pi(self, x, k, info=None):
        ...
        """
        alt_adj = self.pid_alt.pi( -(np.abs(x[0])- x[1]) ) #!b
        ang_adj = self.pid_ang.pi( -((.25 * np.pi) * (x[0] + x[2]) - x[4]) ) #!b Compute the alt_adj and ang_adj as in the gitlab repo (see code comment).
