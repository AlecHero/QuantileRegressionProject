...
def boeing_simulation():
    ...
    Q, R, q = compute_Q_R_q(model, dt) #!b #!b Compute Q, R and q here
    agent = LQRAgent(env, A=A, B=B, d=d, Q=Q, R=R, q=q) #!b;halfquestionmarks #!b Use your LQRAgent to plan using the system matrices.
...
def compute_A_B_d(model : ControlModel, dt : float):
    ...
    B_discrete = scipy.linalg.inv(model.A) @ (A_discrete - np.eye(model.A.shape[0])) @ model.B #!b
    d_discrete = scipy.linalg.inv(model.A) @ (A_discrete - np.eye(model.A.shape[0])) @  d #!b
