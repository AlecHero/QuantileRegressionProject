...
class ConstantLQRAgent(LQRAgent):
    def pi(self,x, k, info=None): #!b
        action = self.L[0] @ x + self.l[0]
        return action #!b Complete this agent here. You need to update the policy-function: def pi(self, ..).

def get_Kp_Kd(L0):
    Kp, Kd = (-L0).flat  #!b #!b Use lqr_agent.L to define Kp and Kd.
    return Kp, Kd
