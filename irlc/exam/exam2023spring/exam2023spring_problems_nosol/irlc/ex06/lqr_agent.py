...
class LQRAgent(Agent):
    def __init__(self, env : ControlEnvironment, A, B, Q, R, d=None, q=None):
        ...
        (self.L, self.l), _ = LQR(A=[A]*N, B=[B]*N, d=[d]*N if d is not None else None, Q=[Q]*N, q=[q]*N if q is not None else None, R=[R]*N) #!b;halfquestionmarks #!b
        self.dt = env.dt
        super().__init__(env)
    ...
    def pi(self,x, k, info=None):
        ...
        """
        u = self.L[k] @ x + self.l[k]  #!b #!b Compute current action here
        return u
