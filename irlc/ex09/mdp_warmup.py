...
def value_function2q_function(mdp : MDP, s, gamma, v : dict) -> dict: 
    ...
    q_dict = {}
    for a in mdp.A(s):
        q_dict[a] = sum([p * (r + (gamma * v[s_] if not mdp.is_terminal(s_) else 0)) for (s_, r), p in mdp.Psr(s, a).items()])
    return q_dict


def expected_reward(mdp : MDP, s, a) -> float:
    transition_p = mdp.Psr(s, a)
    expected_reward = sum([p * r for (s_, r), p in transition_p.items()])
