...
class WindyRoomEnv(Env):
    metadata = {
        "render_modes": ["human", "rgb_array", "ansi"],
        "render_fps": 4,
    }

    def __init__(self, render_mode: str | None = None, p_random: float = 0.1, shape=(10, 14)):
    ...
    def _calculate_transition_prob(
        ...
        for delta in deltas:
            ...
            winded_position = new_position + np.array(POSITION_MAPPING[UP]) * self._wind[tuple(new_position)]
            ...
            p_trans = self.p_random / (len(deltas) - 1)

            if delta == deltas[0]:
                outcomes.append((1.0 - self.p_random, winded_state, 0, False))
                outcomes.append((p_trans, new_state, 0, False))
            elif self._cliff[tuple(new_position)]:
                outcomes.append((p_trans, old_state, 0, False))
            elif tuple(new_position) == (self.shape[0] - 1, self.shape[1] - 1): # Goal state
                outcomes.append((p_trans, new_state, 1, True))
            else:
                outcomes.append((p_trans, new_state, 0, False))
